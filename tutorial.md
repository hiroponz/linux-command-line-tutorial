...
「ls」コマンドで利用できる「オプション」や「引数」の指定方法を調べるために、次のコマンドを実行してください。

```bash
man ls
```

「man」はmanualの略で、コマンドのユーザーマニュアルを表示するコマンドです。

ターミナルには、次のように表示されるはずです。
...
一般的に、「man」コマンドの内容は、1つの画面には収まらないので、キーボードを使用して画面をスクロールする仕組みが組み込まれています。この仕組みのことを「pager」と呼びます。

「pager」の主な使用方法は次の通りです。

- スペース: 画面を1ページ下へスクロールする
- u: 画面を半ページ上へスクロールする
- j: 画面を1行下へスクロールする
- k: 画面を1行上へスクロールする
- /: 検索（キーワードを入力してEnterを押す）
- n: 次の検索候補を表示する
- N: 前の検索候補を表示する
- q: 「pager」を終了する
- h: 「pager」のヘルプを表示する

「NAME」にはコマンド名と概要が記載されています。
「SYNOPSIS」にはコマンドのオプションと引数の指定方法が記載されています。
「DESCRIPTION」にはコマンドの詳細な説明が記載されています。画面の下へスクロールすると、使用可能なオプションを確認できます。

ここで「SYNOPSIS」読み方を簡単に説明します。
「ls」コマンドの「SYNOPSIS」は次のように記載されています。
...
まずは、「ls」コマンドはオプションと「FILE」という引数を指定できることが分かります。
ここで「[]」で囲まれていることは、そのオプションや引数が省略可能であることを意味しています。
また、「...」はオプションや引数を複数指定できることを意味しています。

続いて、ファイルをコピーする「cp」コマンドのマニュアルを見てみましょう。

```bash
man cp
```

「cp」コマンドの「SYNOPSIS」は次のように記載されています。
...
これは、「cp」コマンドのオプションと引数の指定方法には3つのパターンがあることを意味しています。

1行目の「`cp [OPTION]... [-T] SOURCE DEST`」を例に読み方を説明します。
「`[OPTION]... [-T]`」は、複数の省略可能なオプションを指定できることを意味します。ただし、「-T」オプションはオプションの最後に指定する必要があります。
「`SOURCE DEST`」は引数として「SOURCE（コピー元）」と「DEST（コピー先）」を指定することができ、省略できないことを意味しています。

以上のようにして、コマンドの使用方法が分からない場合は、マニュアルを調べるクセを付けることが上達への近道です。
